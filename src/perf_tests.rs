...
pub fn matmul_transb_serial(
    ...
    for i in 0..m {
        for j in 0..n {
            let mut sum = 0.0;
            for p in 0..k {
                sum += a_data[i * k + p] * b_data[j * k + p] as f32;
            }
...
fn story() ->  usize{
    ...
    let result = format!("{:?}{:?}", input, tokenizer.decode(&output_ids, true).unwrap());
    print!("{:?}", input);
    result.len()
}
...
mod tests {
    ...
    fn bench_matmul_transb() {
        ...
        println!("matmul_transb_parallel: {:?} for 100 runs", duration_parallel);
    ...
    fn generate_test() {
        ...
        println!("{:?} token/s for 1 runs", ans_size as f32 / duration.as_secs_f32());
    }
}